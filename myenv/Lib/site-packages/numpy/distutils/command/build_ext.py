...
class build_ext(old_build_ext):
    ...
    def run(self):
        ...
        for ext in self.extensions:
            ...
            if macros:
                log.info(
                    "extending extension %r defined_macros with %r" % (ext.name, macros)
                )
        ...
        pkg_roots = {
            self.get_ext_fullname(ext.name).split(".")[0] for ext in self.extensions
        }
    ...
    def build_extension(self, ext):
        ...
        if fcompiler is not None:
            fcompiler.extra_f77_compile_args = (
                (ext.extra_f77_compile_args or [])
                if hasattr(ext, "extra_f77_compile_args")
                else []
            )
            fcompiler.extra_f90_compile_args = (
                (ext.extra_f90_compile_args or [])
                if hasattr(ext, "extra_f90_compile_args")
                else []
            )
        ...
        if (f_sources or fmodule_sources) and fcompiler is None:
            raise DistutilsError(
                "extension %r has Fortran sources "
    ...
    def _add_dummy_mingwex_sym(self, c_sources):
        ...
        objects = self.compiler.compile(
            [os.path.join(build_src, "gfortran_vs2003_hack.c")],
            output_dir=self.build_temp,
        )
    ...
    def _libs_with_msvc_and_fortran(self, fcompiler, c_libraries, c_library_dirs):
        ...
        for libname in c_libraries:
            ...
            if fileexists:
                continue
            log.warn(
                "could not find library %r in directories %s"
    ...
    def get_outputs(self):
        ...
        for ext in self.extensions:
            ...
            fullname = self.get_ext_fullname(ext.name)
            outputs.append(
                os.path.join(self.build_lib, self.get_ext_filename(fullname))
            )
        return outputs
